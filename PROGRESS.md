...
**目标**：让用户把低优先级会话移出日常侧边栏，并能在个人设置中按原知识空间查找和恢复。

- [x] 会话归档与撤销：活动列表不再提供物理删除，处理中会话禁止归档
- [x] 会话置顶与取消置顶：支持多会话置顶，置顶区按最近置顶排序，普通区按更新时间排序
- [x] 会话归档状态持久化；活动会话查询自动排除已归档数据
- [x] 个人设置入口与 `/settings/archived-chats` 归档中心
- [x] 归档会话按原知识空间分组，支持标题搜索、空间筛选与恢复
- [x] 原空间已归档时，先恢复空间再恢复会话
- [x] 租户与用户范围的归档、恢复、分页查询 API，以及幂等和越权保护
- [x] 前后端单元测试、浏览器端归档/恢复回归和 CI 闸门
