...
components:
  ...
  session-row:
    ...
    rounded: '{rounded.sm}'
    padding: 5px 8px
  # 边取 rule 而非 rule-strong：轮廓由 contact 的接触影加这道浅边合力交代。
  # 重边 + 小圆角 = 僵硬
  composer:
...
| 纸 | {elevation.contact} | 压在桌上，0.1mm 厚 | 主区那张纸、卡片 |
| 调阅单 | {elevation.slip} | **笔下那张**，压在别的纸上面 | 输入器 |
| 浮层 | {elevation.overlay} | 唯一真正离开桌面的东西 | 下拉、菜单、弹窗 |

调阅单这一级不是给输入器加戏：**归档的纸平放，手边正在写的那张压在上面**——桌上真的有这一层。但它的偏移封顶 {motion.travel}（2px 实际取值），因为它是**压着**，不是飘着。

**"不浮"针对的是动作，不是存在感。** 输入器聚焦时不该跳一下（见 Motion），但它有影——一张压在别的纸上面的单子当然有影。把这两件事混起来，就会把影一起砍掉，然后这张单子就从桌上消失了。
...
**抬头只在新建对话时出现。** 空间是会话的属性，在建立那一刻定下来：填单子的时候要写库房，单子交上去之后那一栏就不会再改了。打开一条历史会话时它已经属于某个空间——侧栏里就嵌在那个空间底下——**再显示一遍是把一次性的选择变成永久的装饰**。没选空间时抬头写「选择工作空间」，选了就写空间名，左侧的叉用来撤销选择。

**两层同圆角（{rounded.xl}），抬头不描边。** 这两条是"僵硬"与否的全部：抬头是压在后面的一个形状，靠底色就认得出来，给它描一圈轮廓只会在接缝处多画一条线；两层圆角不一致，叠起来的接缝就是歪的。

**边取 {colors.rule}，不取 {colors.rule-strong}。** 交代轮廓的是 {elevation.slip} 的影加这道浅边，两者合起来就够。重边配小圆角，就是那种"塑料感"。

**它压在纸上，用 {elevation.slip}——但聚焦时不抬起来。** "不浮"说的是动作：这张单子不该因为你点了它就跳一下。影是另一回事，那是它本来就压在别的纸上面。

聚焦时切到 {components.composer-focused}：光标出现，边从 {colors.rule} 补到 {colors.rule-strong}，**再无别的动静**。{components.composer-send} 是石墨实心的圆键——**不是朱红的**。发送不是出处。
...
- **位移**：最大 {motion.travel}（3px），而且**只有浮层配用**——它是唯一真正离开桌面的东西，进出时上浮 3px 加淡入。侧栏项 hover 是底色淡入，**不位移**；输入器聚焦**什么也不动**：它平放在纸上，聚焦不该让它跳一下。
