...
- **纸与桌板之间不再画边框。** {elevation.contact} 已经是那条边了，再加 {colors.rule} 就成了双线。卡片压在纸上则仍需 {colors.rule}——同色叠同色，光靠接触影分不开。
...
- **Don't** 把纸的接触影调成大模糊的投影（>4px blur / >0.08 alpha）。那是纸飘在半空，不是压在桌上。
