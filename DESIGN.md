...
| 纸 | {elevation.contact} | 压在桌上，0.1mm 厚 | 主区那张纸、卡片、输入器 |
| 浮层 | {elevation.overlay} | 唯一真正离开桌面的东西 | 下拉、菜单、弹窗 |

**桌上只有两种东西：贴着的，和被拿起来的。** 中间没有第三种。曾经有过一级"调阅单"，给输入器用一片更厚的影——但那是在给它加戏：调阅单也是一张纸，它平放在桌上，没有理由半浮着。删掉之后这套梯子才和"纸不浮"真正自洽。
...
**边取 {colors.rule}，不取 {colors.rule-strong}。** 交代轮廓的是 {elevation.contact} 的接触影加这道浅边，两者合起来就够。重边配小圆角，就是那种"塑料感"。

**这张单子平放在纸上，不浮、也不因为聚焦而抬起来。** 它是调阅单，不是浮层——桌上的东西只有贴着和被拿起来两种，它属于前者。
